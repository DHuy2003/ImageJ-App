...
def calculate_rating(citations: int, year: int) -> float:
    """Tính rating dựa trên citations/year"""
    current_year = datetime.now().year
    age = max(1, current_year - year)
    citations_per_year = citations / age
    ...
    if citations_per_year >= 50:
        return 5.0
    elif citations_per_year >= 30:
        return min(5.0, 4.5 + (citations_per_year - 30) / 40)
    elif citations_per_year >= 15:
        return 4.0 + (citations_per_year - 15) / 30
    elif citations_per_year >= 5:
        return 3.5 + (citations_per_year - 5) / 20
    elif citations_per_year >= 2:
        return 3.0 + (citations_per_year - 2) / 6
    else:
        return max(1.0, 2.5 + citations_per_year / 4)
...
def parse_pubmed_article(article: Dict) -> Optional[Article]:
    ...
    return Article(
        ...
        year=year,
        citations=citations,
        rating=calculate_rating(citations, year),
        doi=doi,
...
def sort_articles(
    ...
    if sort_by == 'citations':
        ...
        return sorted(articles, key=lambda x: x.get('rating', 0), reverse=reverse)
